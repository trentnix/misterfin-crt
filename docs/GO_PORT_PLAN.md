...
The Go implementation now includes a framebuffer prototype and a Ghostty browsing path with configuration, authentication, paginated lists, and artwork. The current C source and tests remain the behavioral reference. See [the build record](GO_BUILD.md) and [the browsing record](GO_BROWSING.md) for validation and remaining limits. Ignored configuration, tokens, build products, and other untracked files were not copied by the clone.
