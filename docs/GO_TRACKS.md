...
The Go-specific `vf_misterfin` filter owns CRT scaling and a retained planar source frame. Original scales the full picture and adds black bars. Zoom takes a centered horizontal, vertical, or four-edge crop before the same single scaling pass. Two reusable scaler contexts avoid rebuilding scaling state on repeated toggles. Source rows are aligned for ARM. Cached input supports paused redraws without decoding another frame. The filter preserves source timestamps and keeps framebuffer geometry fixed, so mode changes do not reopen output or change the audio clock. Live TV uses the same filter and keeps its existing audio synchronization settings.
