...
Picture offers Original and Zoom for recorded video and for Live TV on MiSTer and inline Ghostty. Original is the default and preserves the complete encoded frame at its display aspect ratio. Zoom enlarges the center and crops the edges.

For sources wider than 4:3, Zoom crops the sides to fill the screen. For sources narrower than 4:3, Zoom crops the top and bottom. A source near 4:3 receives a fixed 4/3 enlargement that crops all four edges. This last behavior lets a user remove letterboxing encoded within a 4:3 frame. Zoom does not detect black bars, so applying it to native 4:3 content also crops the picture. The saved choice remains available when source metadata changes.
...
For recorded video, the choice survives seeking, audio changes, subtitle changes, and reopening the video. Live TV picture mode lasts for the current playback session and starts at Original when a channel is reopened. Separate-window FFplay still uses the existing stream handoff for picture changes. Applying a different mode dismisses the picker and reloads the stream at the current position. For Live TV, FFplay exposes only Original because it cannot change picture mode without restarting the stream.

`playback.PictureMode` describes the shared choice. MPlayer and the Python helper implement `player.PictureSetter`, which advertises `VideoTracks.LivePicture` to the browser. MPlayer receives `pausing_keep_force misterfin_picture <mode> <request>`. The Python helper receives `picture <mode> <request>`. Both reply with `ANS_PICTURE_MODE=<request>,<mode>`, where -1 reports failure. The browser matches replies to the decoder generation and request, with a handoff fallback for decoders without this capability.

The Go-specific `vf_misterfin` filter owns CRT scaling and a retained planar source frame. Original scales the full picture and adds black bars. Zoom takes a centered horizontal, vertical, or four-edge crop before the same single scaling pass. Reusable contexts for each picture mode avoid rebuilding scaling state on repeated toggles. Resized 480/576-line output scales in YUV before accelerated ARM color conversion. See [ARM color conversion](GO_PLAYBACK.md#arm-color-conversion). Source rows are aligned for ARM. Cached input supports paused redraws without decoding another frame. The filter preserves source timestamps and keeps framebuffer geometry fixed, so mode changes do not reopen output or change the audio clock. Live TV uses the same filter and keeps its existing audio synchronization settings.

Inline libmpv changes its [video-zoom property](https://mpv.io/manual/stable/#options-video-zoom) on the existing square-pixel 4:3 render surface. The helper reads the decoded display aspect ratio and calculates the centered enlargement. Property changes trigger redraws through the existing render callback, including while paused. FFplay crops using the decoded sample aspect ratio. All players apply picture fitting before shared UI composition, so Go-rendered controls and subtitles retain their normal size. Server-burned subtitles remain part of the video and can be cropped.
