...
- **[Noto](https://github.com/notofonts/noto-fonts)** and **[Noto CJK](https://github.com/notofonts/noto-cjk)** — SIL Open Font License 1.1. Unmodified fonts for subtitles and captions are embedded in `internal/caption/fonts.zip`, along with their licenses and a source/checksum manifest. `tools/build_subtitle_fonts.py` rebuilds the archive from pinned revisions. [Noto license texts](licenses/noto.txt).
