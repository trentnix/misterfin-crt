...
Picture mode affects decoded video before overlay composition. The shared `playback.PictureMode` passes through playback handoffs to the selected decoder. Native playback advertises `VideoTracks.LivePicture` and accepts picture controls through the running player. MPlayer's `vf_misterfin` filter fits or crops the retained source frame, then scales once to the fixed CRT canvas. Its acknowledgment updates the active choice without closing the Picture tab. Paused changes reuse the same frame and timestamp. Inline libmpv and FFplay still apply their picture mode at decoder startup. `RasterRenderer` uses the same full-screen panel for Subtitles, Audio, and Picture. Controls and text subtitles retain their normal size. See [picture modes](GO_TRACKS.md#picture-modes).
