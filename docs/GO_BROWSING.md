...
If no servers appear, make sure Jellyfin discovery is enabled and UDP port 7359 can reach the server. Containers must expose that UDP port. Broadcast discovery normally stays on the local subnet. Retry after fixing the network, or set `server.url` in `settings.json` using the [connection example](GO_CONFIGURATION.md#server-connection). A remembered server that becomes unreachable shows a connection failure. Use About → Connections to choose a different connection or start Jellyfin discovery. Automatic address-change recovery is not implemented.
