...
The Continue card uses up to twelve covers from the current feed. Its cover metadata comes from the feed rather than a library-count or mosaic query. Individual images share the artwork memory cache. This changing home collage is not persisted to disk, and its synthetic ID is never sent to Jellyfin as a library or item ID.
