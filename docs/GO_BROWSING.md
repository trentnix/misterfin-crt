...
Item details fetch the overview, year, rating, runtime, and watched/resume state. The screen uses a fading backdrop and a transparent logo when available. Pressing B starts or resumes movies, episodes, videos, and music videos. Desktop playback opens a separate FFplay window. A in Ghostty stops playback and returns to details. See [the playback guide](GO_PLAYBACK.md). The UI has no photo viewer, search, Continue Watching, Next Up, or controller input. About, the setup starfield, exact carousel transition timing, and full C feature parity remain pending. Playback still needs pause, seek, track selection, music, and Live TV. Text supports ASCII and Latin-1. Other characters use a fallback glyph. Artwork loads on selection and uses a bounded session cache. The cache does not persist to disk. The C application remains the reference.
