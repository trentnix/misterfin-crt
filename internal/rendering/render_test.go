...
func TestCRTLayoutAndExitOverlay(t *testing.T) {
	for _, h := range []int{240, 288} {
		...
		m.Content.Page.Items = []jellyfin.Item{{Name: "Movie", Type: "Movie"}}
...
func TestCarouselShowsLibraryName(t *testing.T) {
	m := Scene{Root: true}
	m.Content.Page.Items = []jellyfin.Item{{Name: "Family Cinema", CollectionType: "movies"}}
...
func TestPhotoFitsPhysicalCRTAspect(t *testing.T) {
	for _, height := range []int{240, 288} {
		m := Scene{Root: true}
		m.Root = false
		m.Content = Content{Detail: &jellyfin.Item{ID: "photo", Name: "Portrait", Type: "Photo"}}
