...
func TestStopReleasesOutputBeforeSlowServerCleanup(t *testing.T) {
	for _, blockedPath := range []string{"/Sessions/Playing/Stopped", "/UserItems/item/UserData", "/LiveStreams/Close"} {
		t.Run(blockedPath, func(t *testing.T) {
			...
			go func() {
				returned <- Run(ctx, client, Config{VideoDecoder: nativeplayer.Decoder{Player: player, Width: 640, Height: 240}, AudioDecoder: nativeplayer.Decoder{Player: player, Width: 640, Height: 240}}, Request{Item: media.Item{ID: "item", Type: kind}, AsyncCleanup: fast, Callbacks: Callbacks{AcquireVideo: func() {}, ReleaseVideo: func() { close(released) }, CleanupDone: func() { close(cleaned) }, Position: func(ticks int64) { position <- ticks }}})
			}()
			select {
			case <-position:
...
func TestCleanupDoneRunsWhenPreparationCannotStart(t *testing.T) {
	calls := 0
	err := Run(context.Background(), nil, Config{VideoDecoder: inlineplayer.Decoder{}, AudioDecoder: inlineplayer.Decoder{}}, Request{Item: media.Item{Type: "Movie"}, Callbacks: Callbacks{CleanupDone: func() { calls++ }, Position: func(int64) {}}})
	if err == nil || calls != 1 {
