...
func TestSupervisorAndApplicationLogsRemainIndependent(t *testing.T) {
	...
	parent, err := openStartupDiagnostics(o, true)
	if err != nil {
		t.Fatal(err)
	}
	parent.phase("interlaced-supervisor")
	child, err := openStartupDiagnostics(o, false)
	if err != nil {
		t.Fatal(err)
	}
	child.phase("browser")
...
func TestDisabledStartupDiagnosticsCreateNoLogs(t *testing.T) {
	...
	trace, err := openStartupDiagnostics(launchOptions{browse: true, config: filepath.Join(dir, "jellyfin.conf")}, true)
	if err != nil {
		t.Fatal(err)
	}
